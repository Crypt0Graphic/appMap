import { ILocation } from './ilocation';

export const locations: ILocation[] = [
  {
    name: 'Rektörlük',
    number: 1
  },
  {
    name: 'Mali İşler',
    number: 1
  },
  {
    name: 'Öğrenci İşleri',
    number: 1
  },
  {
    name: 'İngilizce Hazırlık Okulu (Merkez)',
    number: 1
  },
  {
    name: 'Bilgi Merkezi',
    number: 1
  },
  {
    name: 'Bilgi İşlem Merkezi',
    number: 1
  },
  {
    name: 'Kurumsal İletişim',
    number: 1
  },
  {
    name: 'Mühendislik Fakültesi',
    number: 2
  },
  {
    name: 'Sağlık Bilimleri Fakültesi',
    number: 2
  },
  {
    name: 'Tıp Fakültesi',
    number: 2
  },
  {
    name: 'Diş Hekimliği Fakültesi',
    number: 2
  },
  {
    name: 'Sağlık Bilimleri Enstitüsü',
    number: 2
  },
  {
    name: 'Fen Fakültesi (Fizik ve Matematik)',
    number: 2
  },
  {
    name: 'Ticari Bilimler Fakültesi',
    number: 3
  },
  {
    name: 'Uygulamalı Bilimler Yüksek Okulu',
    number: 3
  },
  {
    name: 'Güzel Sanatlar Fakültesi',
    number: 6
  },
  {
    name: 'Eğitim Fakültesi',
    number: 6
  },
  {
    name: 'İletişim Fakültesi',
    number: 6
  },
  {
    name: 'Mimarlık Fakültesi',
    number: 6
  },
  {
    name: 'Fen - Edebiyat Fakültesi',
    number: 6
  },
  {
    name: 'Eğitim Bilimleri Enstitüsü',
    number: 6
  },
  {
    name: 'Yabancı Diller Yüksek Okulu',
    number: 6
  },
  {
    name: 'Hukuk Fakültesi',
    number: 4
  },
  {
    name: 'İktisadi ve İdari Bilimler Fakültesi',
    number: 4
  },
  {
    name: 'Sosyal Bilimler Enstitüsü',
    number: 4
  },
  {
    name: 'Fransızca Hazırlık',
    number: 4
  },
  {
    name: ' Tanıtım Günleri',
    number: 5
  },
  {
    name: 'İstek Servis',
    number: 5
  },
  {
    name: 'Kültür ve Spor Müdürlüğü',
    number: 5
  },
  {
    name: 'Banka',
    number: 5
  },
  {
    name: 'Burçak Gıda',
    number: 5
  },
  {
    name: 'Fitness Center',
    number: 5
  },
  {
    name: 'Kitap Evi',
    number: 5
  },
  {
    name: 'Kuaför',
    number: 5
  },
  {
    name: 'Kapalı ve Açık Yüzme Havuzu',
    number: 5
  }
];
